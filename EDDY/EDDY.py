...
def greet():
    ...
    conn.commit()
    conn.close()
    SR.speak("\nMyself EDDY. How may I help you?")
    ...
    try:
        while(True):
            ...
            #asking for name
                ...
                SR.speak("My name is EDDY and I'm here to help you.")
            #How are you
            ...
            #google, youtube,whatsapp and location
            #playing on youtube
            ...
            #image search
                ...
                if 'of' in query:
                    ...
                    webbrowser.get().open(url)
                    break
            elif there_exists(["what is my exact location","where am i","What is my location","my current location","exact current location"],query):
            ...
            #whatsapp message
            elif there_exists(['open whatsapp messeaging','whatsapp','send a whatsapp message','send whatsapp message','please send a whatsapp message'],query):
            ...
            #play game
            elif there_exists(['would like to play some games','play some game','would like to play some game','want to play some games','want to play game','want to play games','play games','open games','play game','open game'],query):
            ...
            #opening software applications
                ...
                os.startfile(r"C:\Program Files\WindowsApps\Microsoft.Paint_11.2309.30.0_x64__8wekyb3d8bbwe\PaintApp\mspaint.exe")
                break
                ...
                os.startfile(r"C:\Windows\System32\perfmon.exe")
                break
            
            elif there_exists(['open snipping tool','snipping tool','start snipping tool'],query):
                SR.speak("Opening snipping tool....")
                os.startfile(r"C:\Program Files\WindowsApps\Microsoft.ScreenSketch_11.2310.54.0_x64__8wekyb3d8bbwe\SnippingTool\SnippingTool.exe")
                break
            elif there_exists(['open code','open visual studio ','open vs code'],query):
                SR.speak("Openning vs code")
                codepath=r"C:\Users\Md NAYAB ANSARI\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Visual Studio Code\Visual Studio Code.lnk"
                ...
                break
            elif there_exists(['open video player',' media player','KMP player'],query):
                SR.speak("Opening KMP media player")
                os.startfile(r"C:\Users\Md NAYAB ANSARI\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\KMPlayer 64X\KMPlayer 64X.lnk")
                break
            ...
            elif there_exists(['exit','quit','shutdown','shut up','goodbye','shut down','sleep'],query):
                SR.speak("bye bye shutting down")
                sys.exit()
...
if __name__=="__main__":
        ...
        root.geometry("{}x{}+{}+{}".format(920,480,int(root.winfo_screenwidth()/2 - 920/2),int(root.winfo_screenheight()/2 - 450/2)))
        root.resizable(0,0)
        root.title("EDDY")
        root.iconbitmap('Eddy.ico')
        root.configure(bg='#2c4557')
        scrollable_text=scrolledtext.ScrolledText(root,state='disabled',height=18,width=86,relief='sunken',bd=5,wrap=tk.WORD,bg='#add8e6',fg='#800000')
        scrollable_text.place(x=10,y=10)
        mic_img=Image.open("Mic.png")
        #mic_img = mic_img.resize((55, 55), Image.LANCZOS)
        mic_img = mic_img.resize((55, 55), Image.BILINEAR)
        #mic_img=mic_img.resize((55,55),Image.ANTIALIA)
        #mic_img = mic_img.resize((55, 55), Image.NEAREST)
        ...
        Listen_Button.place(x=450,y=390)
        myMenu=tk.Menu(root)
